...
Because this kind of record is easily observed by the environment and it represents a single interaction, it is often used to build models of CSP processes. As a matter of fact, there is one named after itself: the $ \mathit{\traces} $ model, represented by the symbol $ \tmodel $. This model is one of the three major denotational models of CSP, the other ones being the \emph{stable} $ \mathit{\failures} \ \fmodel $ and the $ \mathit{\failures \mhyphen \divergences} $ model $ \nmodel $. It defines the meaning of a process expression as the set of sequences of events (traces) that the process can be observed to perform.
...
\begin{table}[htb]
	\begin{center}
		\caption[CSP constructors and their corresponding ASCII representations]{CSP constructors and their corresponding ASCII representations.}
		\label{tab:csp-cspm}
		\begin{tabular}{ |l|c|c| }
			\hline
			Constructor & CSP & \CSPM{} \\ [0.5ex]
			\hline
